...
SCHEDULER.every '4h', :first_in => 0 do |job|
  j = httpGetter("#{server}/repos/uc-cdis/#{repo}/pulls\?state\=closed")
  total_prs = 0
  total_failures = 0
  last_force_merge = ""
  j.each_with_index { |idx, pr|
    ...
    # only PRs from the last 48hs
    puts("updated_at: #{updated_at}")
    puts("48hs ago: #{Time.now - (3600 * 48)}")
    if updated_at >= Time.now - (3600 * 48)
      total_prs = total_prs + 1
      j2 = httpGetter("#{server}/repos/uc-cdis/#{repo}/pulls/#{j[pr]['number']}")
      j3 = httpGetter("#{j2['statuses_url']}")
      puts("state: #{j3[0]['state']}")
      if j3[0]['state'] != 'success'
        total_failures = total_failures + 1
        if j2.has_key? 'merged_at' and j2['merged_at'] != null
  ...
  send_event('arborist_ci_stats', { items: stats.values } )
end
